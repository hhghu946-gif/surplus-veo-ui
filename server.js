...
function familyOf(id = "") {
  ...
  if (x.includes("nano-banana-2")) return "nano-banana-2";
  if (x.includes("nano-banana-pro")) return "nano-banana-pro";
  return null;
}

async function getNanoBananaCatalog() {
  const response = await fetch(`${API_URL}/v1/models`);
  if (!response.ok) throw new Error(`Could not load Surplus model catalog (${response.status}).`);
  const json = await response.json();
  const rows = Array.isArray(json?.data) ? json.data : [];
  return rows.filter(m => familyOf(m?.id)).map(m => ({
    id: m.id,
    family: familyOf(m.id),
    isEdit: String(m.id).toLowerCase().includes("-edit"),
    inputModalities: m.architecture?.input_modalities || []
  }));
}

function chooseImageModel(catalog, family, needsEdit) {
  const candidates = catalog.filter(m => m.family === family);
  if (needsEdit) {
    return candidates.find(m => m.isEdit) ||
      candidates.find(m => Array.isArray(m.inputModalities) && m.inputModalities.includes("image")) ||
      candidates[0];
  }
  return candidates.find(m => !m.isEdit) || candidates[0];
}

app.get("/api/config", (_req, res) => res.json({ defaultVideoModel: DEFAULT_VIDEO_MODEL }));

app.get("/api/image-models", async (_req, res) => {
  try {
    const catalog = await getNanoBananaCatalog();
    res.json({
      models: catalog,
      families: {
        "nano-banana-pro": catalog.filter(m => m.family === "nano-banana-pro").map(m => m.id),
        "nano-banana-2": catalog.filter(m => m.family === "nano-banana-2").map(m => m.id)
      }
    });
  } catch (err) {
    res.status(500).json({ error: err.message || "Unable to load image models." });
  }
});
...
app.post("/api/generate", videoUpload.single("image"), async (req, res) => {
  try {
    ...
    const body = { model, prompt, image_url: dataUri(req.file), aspect_ratio: aspectRatio };
...
app.post("/api/generate-image", imageUpload.array("images", 8), async (req, res) => {
  try {
    ...
    const prompt = String(req.body.prompt || "").trim();
    const family = String(req.body.model_family || "nano-banana-pro").trim();
    const aspectRatio = String(req.body.aspect_ratio || "16:9").trim();
    const resolution = String(req.body.resolution || "2K").toUpperCase();
    const files = Array.isArray(req.files) ? req.files : [];

    if (!prompt) return res.status(400).json({ error: "Prompt is required." });
    if (!["nano-banana-pro","nano-banana-2"].includes(family)) return res.status(400).json({ error: "Unsupported image model." });
    if (!["1K","2K","4K"].includes(resolution)) return res.status(400).json({ error: "Resolution must be 1K, 2K or 4K." });
    if (files.some(f => !f.mimetype.startsWith("image/"))) return res.status(400).json({ error: "All reference files must be images." });

    const totalRawBytes = files.reduce((sum, f) => sum + f.size, 0);
    if (totalRawBytes > 7 * 1024 * 1024) {
      return res.status(413).json({ error: "Combined reference images are too large. Keep the total under about 7 MB." });
    }

    const catalog = await getNanoBananaCatalog();
    const chosen = chooseImageModel(catalog, family, files.length > 0);
    if (!chosen?.id) return res.status(400).json({ error: `No live Surplus model found for ${family}.` });
    ...
    const result = await surplus(endpoint, {
      method: "POST",
      headers: { "Content-Type": "application/json", "Idempotency-Key": crypto.randomUUID() },
      body: JSON.stringify(body)
    });
    ...
    res.json({ created: result?.created, model: chosen.id, usedReferences: files.length, data });
  } catch (err) {
